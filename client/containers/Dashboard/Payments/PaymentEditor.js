...
export default class DeliveryEditor extends Component {
  ...
  submit = e => {
    e.preventDefault();
    this.props.onSubmit(this.state.name, this.state.url, this.state.price, this.state.icon);
  }

  render() {
    const { name, url, price, icon } = this.state;
    ...
    return (
      <Form>
        ...
        <Form.Field>
          <Form.Label>
            Payment Url 
            <Form.Input type="text" placeholder="Url" value={url} onChange={this.changeUrl}/>
          </Form.Label>
        </Form.Field>
        <Form.Field>
          <Form.Label>
